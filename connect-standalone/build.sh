...
function usage () {
    echo "$0: $1" >&2
    echo
    echo "Usage: $0 [--build] [--push] [--user ueisele] [--github-repo apache/kafka] [ [--commit-sha b172a0a] [--tag 3.4.1] [--branch trunk] [--pull-request 9999] ] [--openjdk-release 17] [--openjdk-version 17.0.7]"
    echo
    return 1
}
...
function build_image () {
    ...
    docker build \
        ...
        echo -t "$(kafka_connect_standalone_image_name ${DOCKERREGISTRY_USER} ${KAFKA_GITHUB_REPO}):${tag}"
        done) \
...
function push_image () {
    ...
    local kafka_tags=($(kafka_connect_image_tags ${KAFKA_TAG_VERSION} $(echo ${KAFKA_GIT_COMMIT_SHA} | cut -c 1-7) ${openjdk_tags[@]}))
    for tag in ${kafka_tags[@]}; do
        docker push "$(kafka_connect_standalone_image_name ${DOCKERREGISTRY_USER} ${KAFKA_GITHUB_REPO}):${tag}"
    done
}
