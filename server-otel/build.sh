...
function build_image () {
    ...
    docker build \
        ...
        echo -t "$(kafka_server_otel_image_name ${DOCKERREGISTRY_USER} ${KAFKA_GITHUB_REPO}):${tag}"
        done) \
...
function push_image () {
    ...
    local kafka_tags=($(kafka_server_image_tags ${KAFKA_TAG_VERSION} $(echo ${KAFKA_GIT_COMMIT_SHA} | cut -c 1-7) ${openjdk_tags[@]}))
    for tag in ${kafka_tags[@]}; do
        docker push "$(kafka_server_otel_image_name ${DOCKERREGISTRY_USER} ${KAFKA_GITHUB_REPO}):${tag}"
    done
}
